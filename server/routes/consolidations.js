...
const UpdateBody = z.object({
  title: z.string().optional(),
  status: z.enum(["loaded","to_customs","released","kg_customs","delivered","closed"]).optional(),
...
export default async function consolidationsRoutes(app) {
  ...
  app.get("/", async (req, reply) => {
    try {
      ...
      const rows = status
        ? await db.select().from(consolidations).where(eq(consolidations.status, status))
        : await db.select().from(consolidations).orderBy(consolidations.createdAt);
      return rows;
    } catch (e) {
  ...
  app.get("/:id", async (req, reply) => {
    const { id } = req.params;
    try {
      const [cons] = await db.select().from(consolidations).where(eq(consolidations.id, id));
      if (!cons) return reply.notFound("Consolidation not found");
      const links = await db.select().from(consolidationPl).where(eq(consolidationPl.consolidationId, id));
  ...
  app.patch("/:id", async (req, reply) => {
    ...
    try {
      ...
      const [before] = await db.select().from(consolidations).where(eq(consolidations.id, id));
      ...
      if (body.status) {
        ...
        if (fromIdx === -1 || toIdx === -1 || toIdx < fromIdx) {
          return reply.badRequest(`Недопустимый переход статуса: ${before.status} → ${body.status}`);
        }
        try { await assertPLsNotBehind(db, id, body.status); }
        catch (e) { return reply.badRequest(e.message); }
      }

      const [after] = await db.update(consolidations)
        .set({
  ...
  app.delete("/:id", async (req, reply) => {
    const { id } = req.params;
    try {
      const res = await db.delete(consolidations).where(eq(consolidations.id, id)).returning();
  ...
  // ADD PL
  app.post("/:id/pl", async (req, reply) => {
    const { id } = req.params;
    try {
      const plId = Number(req.body?.plId);
      if (!Number.isInteger(plId)) return reply.badRequest("plId (number) required");
      try { await ensureAllPLsAreToLoad(db, [plId]); }
      catch (e) { return reply.badRequest(e.message); }
      await db.insert(consolidationPl).values({ consolidationId: id, plId }).onConflictDoNothing();
  ...
  // REMOVE PL
  app.delete("/:id/pl/:plId", async (req, reply) => {
    const { id, plId } = req.params;
    try {
      await db.delete(consolidationPl)
        .where(eq(consolidationPl.consolidationId, id))
        .where(eq(consolidationPl.plId, Number(plId)));
  ...
  app.get("/:id/status-history", async (req, reply) => {
    const { id } = req.params;
    try {
      const rows = await db.select().from(consolidationStatusHistory)
